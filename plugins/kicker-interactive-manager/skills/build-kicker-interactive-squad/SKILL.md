...
- Das `defender_architecture`-Audit als harte Untergrenze prüfen. Bei sieben Abwehrplätzen sind für `gering` höchstens vier Mindestpreisspieler insgesamt und höchstens einer in der Startelf zulässig. Mindestens alle bis auf einen startenden Verteidiger müssen `minutes` und `role` jeweils mindestens 65, `fitness` mindestens 65 sowie Transfer-, Verletzungs-, Rotations- und Rollenrisiken innerhalb der Modellgrenzen erreichen. Ein Verstoß muss weiter optimiert werden oder den Lauf abbrechen; eine Warnung allein genügt nicht.
