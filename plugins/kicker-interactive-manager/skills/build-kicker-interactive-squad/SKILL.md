...
Die Kombination `verlässlich` und `gering` ist das konservative Kollegenprofil: Der Kader soll über einen möglichst sicheren, hochwertigen Kern funktionieren. Das vollständige Budget ist jedoch immer zuerst zu verwenden; `wartungsarm` darf niemals Restbudget erzeugen. Danach konzentriert das Modell so viel Wert wie sinnvoll auf die stärkste Elf. Es verlangt dort mindestens 55 Prozent des Kaderwerts und strebt 80 Prozent an, begrenzt dieses Ziel aber automatisch auf die anhand der aktuellen Positionspreise überhaupt erreichbare Startelf-Obergrenze der Liga. Bei vollem Budget darf es Bankwert nur über einen kostenneutralen Doppeltausch in die Startelf verschieben: Reserve günstiger und einen echten Starter um denselben Betrag aufwerten. Die Startelf enthält mindestens zwei Stürmer sowie höchstens vier Verteidiger und der Kader nur einen Torwartblock mit belastbarer vereinsinterner Nummer eins. Im verlässlichen Profil muss die Startelf zudem mindestens einen offensiven Premiumanker enthalten, der ausschließlich aus mehrjähriger Leistung, Rolle, Stabilität und aktueller Einsatzsicherheit abgeleitet wird; Bekanntheit, `benchmark` oder frühere Nutzernennung geben keinen Auswahlbonus. Für den Torwart gelten mindestens 70 Prozent Saison-Stammplatzwahrscheinlichkeit, höchstens 40 Prozent Risiko eines noch kommenden externen Stammkeepers und mindestens mittlere Hierarchiesicherheit. Offene Torwartduelle sind für `gering` kein zulässiges Glücksspiel.
