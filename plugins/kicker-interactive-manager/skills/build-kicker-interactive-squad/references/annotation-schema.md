...
- Die letzte historische Vereinszuordnung mit dem aktuellen Kicker-Verein vergleichen. Bei einem Vereinswechsel bleibt die portable Grundqualität bestehen, aber positive wie negative Form wird nur reduziert auf `role` und `context` übertragen und `unknown_role` erhöht. Das ist ein Kontextreset, keine pauschale Bestrafung jedes Transfers. Eine tatsächliche taktische Systempassung nur mit eigenständiger aktueller Evidenz behaupten.
...
- Vorbereitung in `preseason_summary` getrennt dokumentieren. Das Objekt enthält mindestens Abdeckung, Klassifikation, Konfidenz, Einsätze, Starts, Minuten, Scorer, Verfügbarkeits-, Rollen-, Leistungs-, Gegner- und Gesamtsignal, wirksamen Verfallsfaktor, angewandtes Gewicht, Bereitschaftsänderung und Talentstatus.
