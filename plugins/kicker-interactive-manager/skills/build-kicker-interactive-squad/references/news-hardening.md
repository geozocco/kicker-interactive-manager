...
4. Ein versionierter zentraler redaktioneller Belegkanal in `config/news/editorial-transfer-evidence.json` trennt Fundstelle und Interpretation. Er nimmt aktuelle Meldungen von Vereinen, Liga, kicker, Sky, Transfermarkt und weiteren belastbaren Redaktionen mit Kicker-ID, Verein, Zeitpunkt und URL auf. Die Pipeline validiert Identität und Aktualität und normalisiert den Status anschließend deterministisch.
5. Offizielle Vereins-, Liga- und Transfermeldungen bleiben der gezielte Fallback für fehlende Zuordnungen, Konflikte und besonders folgenreiche Meldungen.
...
- Ein ausreichend belastbarer fortgeschrittener Abgang bleibt rechtlich und fachlich `advanced`, sperrt aber vorsorglich die Endauswahl. Das ist kein behaupteter Vollzug: Der Optimierer entfernt den Kandidaten vor der Berechnung, optimiert aus dem freigegebenen Pool neu und dokumentiert den Vorgang im finalen News-Gate.
- Ein kritischer Einzelspieler darf erst nach zwei unabhängigen Suchdurchgängen als „kein aktuelles Signal gefunden“ gelten. Diese negative Recherche läuft nach zwei Stunden ab und behält 15 Punkte Restrisiko. Scheitert oder widerspricht die Zweitprüfung, gelten 45 Punkte Transferrisiko und der Spieler bleibt für eine finale Empfehlung gesperrt. Ein unsicheres Suchergebnis wird damit niemals als Risiko null ausgegeben.
...
Die OpenAI-Rollenrecherche bezieht jeden aktuell verfügbaren Spieler der zentralen Kicker-Marktliste ein. Die bisherige feste Obergrenze entfällt. Offene Rollenfälle, Benchmarks, bestätigte Ankerkandidaten, Torwarthierarchien, hochpreisige Offensivspieler und qualifizierte U23-Potenzialspieler bestimmen weiterhin die Reihenfolge und Dringlichkeit, nicht mehr die Teilnahme. Sie erfasst neben Startwahrscheinlichkeit und Standards auch Trainervertrauen, Kaderstatus, taktische Passung, Konkurrenz um den Platz, erwartetes Minutenband und Rollenstabilität. Diese Umfeldsignale bleiben von Verletzung, Transfer, Vorbereitung und historischer Form getrennt, um Doppelzählung zu vermeiden. Sie verwendet standardmäßig `gpt-5.6-luna`; kritische Fälle laufen mit niedriger Reasoning-Stufe, routinemäßige Extraktionen ohne zusätzliche Reasoning-Stufe. Statische Anweisungen und Strukturschema verwenden einen expliziten GPT-5.6-Prompt-Cache-Präfix, während Datum und Spielerliste außerhalb des Cache-Präfixes bleiben. Ergebnisse besitzen getrennte Fristen: offene Torwartfragen werden nach drei Tagen, übrige Torwartrollen nach sieben Tagen und stabile Feldspielerrollen nach vierzehn Tagen erneut geprüft. Bis zum nächsten Prüfzeitpunkt entstehen keine neuen Modellaufrufe. Ändert sich Name, Verein oder Position eines bereits bekannten Spielers, wird der Cache über einen Kader-Fingerprint sofort ungültig. Auch ein korrektes Ergebnis „kein belastbares aktuelles Rollensignal“ wird drei beziehungsweise sieben Tage separat gecacht, ohne daraus eine Spielerrolle zu erfinden. Wird ein Spieler danach im Qualitätsbestand als `role_research.required` eingestuft, übersteuert dieser Pflichtstatus einmalig den negativen Cache und erzwingt im nächsten Lauf eine gezielte Suche nach Transferankündigung, Trainer-/Sportdirektorzitat sowie aktuellen Vorbereitungs- oder Aufstellungsbelegen. Bleibt die Recherche ergebnislos, wird ausschließlich dieser Spieler im finalen Optimierungslauf als vorübergehend nicht auswählbar dokumentiert; Shortlists zeigen ihn weiterhin und der übrige belegte Ligapool wird nicht blockiert. Fehlt ein Spieler in einer Modellausgabe oder scheitert seine Belegnormalisierung, wird er ausdrücklich als `research_inconclusive` gespeichert und bereits im nächsten Lauf erneut geprüft; dieser Status ist niemals ein sportlicher Malus. Ein noch gültiges altes Profil bleibt bei einem vorübergehenden API-Fehler als gekennzeichneter Fallback erhalten; ohne gültigen Beleg greift die bestehende deterministische Rollenheuristik. Das Snapshot-Audit `role_research` nennt Modell, vollständige Zielzahl, Positions- und Vereinsabdeckung, Cachetreffer, zurückgestellte Routineziele, erzwungene Wiederholungen, neue Profile, belegte Enthaltungen, unklare Ergebnisse, Requests, echte Token-/Cache-Nutzung, Web-Search-Aufrufe, geschätzte Kosten und Fehler, aber niemals Schlüssel oder vollständige Modellprompts.

Zusätzlich speichert die Rollenrecherche die jüngsten Pflichtspielstarts, direkte Konkurrenzentscheidungen, Operation/Reha/Wiedereingliederung und konkrete Abgangslagen als getrennte belegte Felder. Diese aktuellen Felder erzeugen deterministische Wahrscheinlichkeitsdeckel, Risiko-Untergrenzen und gegebenenfalls eine vorübergehende Auswahlsperre. Schweigen einer Quelle erzeugt niemals einen Status; die jüngste direkte Trainerentscheidung besitzt jedoch Vorrang vor bloßen Saisonminuten.
...
Der Transfer-Watcher deckt alle verfügbaren Marktspieler ab, recherchiert aber veränderungs- und risikobasiert. Neue Spieler, bestehende Transfermeldungen, hohe Transferrisiken und offene Rollenfälle werden zuerst behandelt. Als `critical` gelten nur aktive Transfergefahr ab 55 Punkten, ein Auswahl-/Ausschlussflag, mindestens 70 Punkte bereits modelliertes Transferrisiko oder eine offene beziehungsweise durch externe Neuverpflichtung bedrohte Torwarthierarchie. Eine bloß fehlende Qualitätsannotation ist dagegen nur `elevated`: Der Qualitätsbestand ist absichtlich kompakter als der Markt und darf deshalb keinen Vollrefresh auslösen. Bereits bestätigte Zugänge mit niedrigem Restrisiko sind ebenfalls nicht kritisch. Kritische Fälle laufen jeweils in einer eigenen Web-Search-Anfrage, damit eine konkrete Meldung nicht im Suchbudget eines Mehrspieler-Batches untergeht. Bleibt ein kritischer Einzellauf leer, folgt automatisch eine zweite Anfrage mit anderen Suchformulierungen und möglichst zwei unabhängigen Domains. Belegte Meldungen bleiben bewusst kurz gecacht: fortgeschrittene beziehungsweise bestätigte Vorgänge sechs Stunden, übrige Gerüchte zwölf Stunden. Ein zweimal belegtes „keine aktuelle Meldung“ wird bei kritischen Spielern zwölf Stunden, bei erhöhtem Risiko 24 Stunden und bei unauffälligen Spielern 72 Stunden gecacht. Ein Wechsel von Name, Verein oder Position erzwingt unabhängig davon eine neue Prüfung. `urgent` hält noch gültige Routinebelege bis zum Standardlauf fest und vermeidet ihre erneute Websuche. Das Modell entdeckt und extrahiert Quellen; Status, Richtung, Ausschlusswirkung und Konfliktbehandlung bleiben deterministisch. Eine ältere Station, die weder den aktuellen Kicker-Verein als Quelle noch als Ziel nennt, wird verworfen. Bei verketteten Wechseln zählt der neueste Schritt, beispielsweise erst der feste Wechsel zum Stammverein und anschließend die Leihe zum aktuellen Kicker-Verein. Bei Leihen werden zusätzlich Stammverein, Dealtyp, Leihzweck und das Niveau der aktuellen Seniorenliga des Stammvereins erfasst. Prestige allein erzeugt dabei keinen Leistungsnachweis.

Das Snapshot enthält außerdem `openai_usage` mit der zusammengefassten und je Stufe getrennten Token-, Prompt-Cache- und Web-Search-Nutzung samt geschätzten Kosten. `evidence_cache` dedupliziert bereits belegte URLs, Aussagen und betroffene Spieler beziehungsweise Vereine als gemeinsamen zentralen Quellenbestand. Die Preisannahmen sind versioniert; für die verbindliche Abrechnung bleibt das OpenAI-Dashboard maßgeblich.
...
`news_audit` im Ergebnis dokumentiert Snapshot-Prüfsumme, Providerstatus, Erzeugungs- und Ablaufzeit, angewandte Spieler, fehlende Zuordnungen, Konflikte und harte Ausschlüsse. `final_selection_gate` muss für alle 22 Spieler `passed` melden. Ein fortgeschrittener Abgang oder eine unklare kritische Recherche wird vorher entfernt und löst eine automatische Neuoptimierung aus. Dieses Audit vor jeder Browseränderung lesen.
