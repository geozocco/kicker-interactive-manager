...
Die OpenAI-Rollenrecherche wählt je Liga bis zu 96 offene Rollenfälle, Benchmarks, bestätigte Ankerkandidaten, hochpreisige Offensivspieler und qualifizierte U23-Potenzialspieler aus. Sie reserviert dabei die zwei wahrscheinlichsten Torhüter sowie mindestens einen offensiven Rollenfall jedes Vereins, bevor weitere Kandidaten nach Priorität aufgefüllt werden. So können viele Feldspieleranker weder die Torwarthierarchien noch kleine Vereine aus dem Recherchebestand verdrängen. Sie verwendet standardmäßig `gpt-5.6-luna` mit niedriger Reasoning-Stufe und Web Search. Ergebnisse besitzen getrennte Fristen: offene Torwartfragen werden nach drei Tagen, übrige Torwartrollen nach sieben Tagen und stabile Feldspielerrollen nach vierzehn Tagen erneut geprüft. Bis zum nächsten Prüfzeitpunkt entstehen keine neuen Modellaufrufe. Auch ein korrektes Ergebnis „kein belastbares aktuelles Rollensignal“ wird drei beziehungsweise sieben Tage separat gecacht, ohne daraus eine Spielerrolle zu erfinden. Ein noch gültiges altes Profil bleibt bei einem vorübergehenden API-Fehler als gekennzeichneter Fallback erhalten; ohne gültigen Beleg greift die bestehende deterministische Rollenheuristik. Das Snapshot-Audit `role_research` nennt Modell, Zielzahl, Positions- und Vereinsabdeckung, Cachetreffer, neue Profile, gecachte Enthaltungen, Requests und Fehler, aber niemals Schlüssel oder vollständige Modellprompts.
